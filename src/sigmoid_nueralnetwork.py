...
class Network(object):
    ...
    def show_misclassified(self, test_data):
        import matplotlib.pyplot as plt

        misclassified = [(x, y) for (x, y) in test_data if np.argmax(self.feedforward(x)) != y]
        for i, (x, y) in enumerate(misclassified[:10]):
            plt.subplot(2, 5, i + 1)
            plt.imshow(x.reshape(28, 28), cmap='gray')
            plt.title(f'True: {y}, Pred: {np.argmax(self.feedforward(x))}')
            plt.axis('off')
        plt.show()
...
test_accuracy = net.SGD(training_data, 30, 10, 3.0, test_data=test_data)
plot_accuracy(test_accuracy)
net.show_misclassified(test_data)
...
def load_image(filename):
    """Load an image file and convert it to a 28x28 grayscale image."""
...
def predict_digit(image_array):
    ...
    output = net.feedforward(image_array)
    predicted_digit = np.argmax(output)
    return predicted_digit

def test_model_with_image():
    """Ask user to upload an image and predict the digit."""
    root = tk.Tk()
...
# Call this function to test the model with an image
